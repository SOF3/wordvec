...
#[test]
fn test_extract_if_shifted_drop_full() {
    test_extract_if_drop(true, 0..5, Some("1"), 1, &["0", "2", "3", "4"]);
}

#[test]
fn test_extract_if_unshifted_drop_full() {
    test_extract_if_drop(false, 0..5, Some("0"), 1, &["1", "2", "3", "4"]);
}

#[test]
fn test_extract_if_shifted_drop_ranged() {
    test_extract_if_drop(true, 1..3, Some("2"), 1, &["0", "1", "3", "4"]);
}

#[test]
fn test_extract_if_unshifted_drop_ranged() {
    test_extract_if_drop(false, 1..3, Some("1"), 1, &["0", "2", "3", "4"]);
}

#[test]
fn test_extract_if_empty_range() {
    #[expect(clippy::reversed_empty_ranges, reason = "testing empty range handling")]
    test_extract_if_drop(false, 3..2, None, 0, &["0", "1", "2", "3", "4"]);
}
